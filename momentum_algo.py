...
def get_1000m_history_data(symbols):
    ...
    calendar_today = calendar.pop().date
    calendar_two_days_ago = calendar.pop(-2).date
    for symbol in symbols:
        minute_history[symbol] = api.polygon.historic_agg_v2(
            symbol=symbol, multiplier=1, timespan='minute', _from=calendar_two_days_ago, to=calendar_today
        ).df.tail(1000)
...
def run(tickers, market_open_dt, market_close_dt):
    ...
    for order in existing_orders:
        if order.symbol in symbols:
            ...
            print('Canceled {} order for {}: Order already existed on watched symbol'.format(order.side, order.symbol))
    ...
    async def handle_trade_update(conn, channel, data):
        ...
        if last_order is not None:
            ...
            if event == 'partial_fill':
                ...
                print('{} order for {} partially filled: {} shares purchased'.format(data.order['side'], symbol, qty))
            elif event == 'fill':
                qty = int(data.order['filled_qty'])
                if data.order['side'] == 'sell':
                    qty = qty * -1
                ...
                print('{} order for {} filled: {} shares purchased'.format(data.order['side'], symbol, qty))
    ...
    async def handle_second_bar(conn, channel, data):
        ...
        if existing_order is not None:
            ...
            if order_lifetime.seconds // 60 > 1:
                ...
                print('Canceled {} order for {}: Timed out'.format(order.side, order.symbol))
            return
        ...
        if (
            ...
            if (
                ...
                stop_price = round(find_stop(data.close, minute_history[symbol], ts), 2)
                stop_prices[symbol] = stop_price

                target_price = round(data.close + ((data.close - stop_price) * 2), 2)
